...
# Compile, test, and create the executable JAR.
.\mvnw.cmd clean package

# Start the packaged application with your configured services available.
java -jar target/HotelBooking-0.0.1-SNAPSHOT.jar
```

On macOS/Linux, use `./mvnw` in place of `.\mvnw.cmd`.

The current suite contains a single `@SpringBootTest` context-loading test. It needs application configuration and a reachable MySQL database; there is no dedicated test profile or in-memory database. It does not verify complete booking, email, authorization, or payment workflows.

For a packaging-only check, use `.\mvnw.cmd clean package -DskipTests`. This skips test execution and is not evidence that the application can start or integrations work.

## Troubleshooting

| Symptom | What to check |
| --- | --- |
| Java release or Lombok compilation errors | Run the build with JDK 21; check `JAVA_HOME` and enable annotation processing in your IDE. |
| Maven Wrapper cannot download files | Check network/proxy access to the Maven repository and the wrapper distribution URL. |
| Unknown database / connection refused / access denied | Create `hotel_booking`, start MySQL, and verify the URL and credentials. |
| Unresolved property at startup | Ensure the local properties file exists and defines `secretJwtString` and the currently injected Stripe property. |
| JWT signing-key error | Supply at least 32 UTF-8 bytes of signing-key material. |
| Protected request fails | Include `Authorization: Bearer ...`; log in again if expired, and use an admin account for admin routes. |
| Booking or payment update fails during email delivery | Verify SMTP settings; the database write may already have succeeded, so check history before retrying. |
| Stripe intent creation fails | Correct the key injection described above and use a valid test secret key. |
| Uploaded image cannot be opened in a browser | The returned value is a filesystem path; no image download route is implemented. |
| Booking email opens a missing page | The hard-coded payment link targets a separate frontend on port 3000. |
| Availability excludes a same-day turnaround | The current overlap query treats touching date boundaries as conflicts. |

## Current limitations

The following describe the current source and should guide further development:

1. **Public role selection:** registration accepts an administrator role from the caller. Restrict role assignment before exposing the service publicly.
2. **Payment verification:** the service uses the wrong key property, trusts client-supplied amounts and success flags, and does not verify booking ownership or signed Stripe events. There is no webhook endpoint or idempotency handling for repeated payment updates.
3. **Booking access matcher:** the booking route pattern lacks its leading slash. Define and verify the intended access policy for reference lookups.
4. **Token expiry mismatch:** the login label says six months while the generated token lasts 7 hours 12 minutes.
5. **Reservation concurrency:** availability is checked before saving, without a locking mechanism to prevent concurrent requests from booking the same room.
6. **Notification consistency:** email methods have `@Async`, but async processing is not enabled in the current application configuration. Booking/payment saves and email delivery are not wrapped in one service transaction, so an email failure can follow a successful database write.
7. **Client integration:** the payment frontend and an HTTP image-serving route are absent; the payment link is hard-coded to localhost.
8. **Validation and account updates:** validation coverage and error mapping are incomplete, and JSON password updates are ignored by the DTO.
9. **Operational tooling:** no migration framework, container setup, CI workflow, pagination, or API documentation UI is provided. CORS currently allows all origins.

Keep local credentials out of version control. The ignored properties file is for local configuration; use environment-backed configuration or a secret store in deployed environments.

## Contributing

Keep changes focused, follow the existing controller/service/repository structure, and update this README when routes, configuration, or behavior changes. Add meaningful tests for behavioral changes and run the build with the required database configuration before submitting a contribution. Do not include credentials or generated room images in commits.

## License

No license file is currently included. Add an explicit license to define permitted reuse and distribution.
